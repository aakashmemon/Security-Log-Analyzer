...
# Track By IP
            ...
            if user_ip in failed_by_user_ip:
                failed_by_user_ip[user_ip] = failed_by_user_ip[user_ip] + 1
            else:
                failed_by_user_ip[user_ip] = 1
...
# User Failure Track Output
    ...
    for user in failed_by_user:
        ...
        if failed_by_user[user] >= 5:
            print("🚨 Critical Activity Detected!")
            critical_users.append(user)
        
        elif failed_by_user[user] >= 3:
            print("⚠ Suspicious Activity Detected!")
            suspicious_users.append(user)
        
        print()

    most_failed_user = max(failed_by_user, key=failed_by_user.get)
    
    print()
    print("==========================")
    print(" Most Failed Login User ")
    print("==========================")
    print()

    print("User:", most_failed_user)
    print("Failed Attempts:", failed_by_user[most_failed_user])
    print()
...
# IP by failure track
    ...
    for ip in failed_by_ip:
        ...
        if failed_by_ip[ip] >= 5:
            print("🚨 Critical IP Detected!")
            critical_ips.append(ip)

        elif failed_by_ip[ip] >= 3:
            print("⚠ Suspicious IP Detected!")
            suspicious_ips.append(ip)

        print()
    
    most_failed_ip = max(failed_by_ip, key=failed_by_ip.get)

    print()
    print("========================")
    print(" Most Failed Login IP ")
    print("========================")
    print()

    print("IP:", most_failed_ip)
    print("Failed Attempts:", failed_by_ip[most_failed_ip])
    print()


    print()
    print("==========================")
    print(" Failed Logins By User/IP ")
    print("==========================")
    print()

    for user_ip in failed_by_user_ip:
        ...
        print("User:", user)
        print("IP:", ip)
        print("Failed Attempts:", failed_by_user_ip[user_ip])
        print()


    most_failed_user_ip = max(
    failed_by_user_ip,
    key=failed_by_user_ip.get
    )

    most_failed_user = most_failed_user_ip[0]
    most_failed_ip = most_failed_user_ip[1]

    print()
    print("==============================")
    print(" Most Failed User/IP Pair ")
    print("==============================")
    print()

    print("User:", most_failed_user)
    print("IP:", most_failed_ip)
    print("Failed Attempts:", failed_by_user_ip[most_failed_user_ip])
    print()
